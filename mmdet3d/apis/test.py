...
def multi_gpu_test(model, data_loader, tmpdir=None, gpu_collect=False):
    ...
    if rank == 0  and model.module.test_cfg.get('with_hidden',None):
        for i, data in enumerate(data_loader):
            with torch.no_grad():
                if len(results) == 0 or data['img_metas'][0].data[0][0]['track']['prev'] == 0:
                    hidden_dict = None
            ...
            if rank == 0:
                ...
                for _ in range(batch_size):
                    prog_bar.update()
    elif not model.module.test_cfg.get('with_hidden',None):
        for i, data in enumerate(data_loader):
            with torch.no_grad():
                result,_ = model(return_loss=False, rescale=True, **data,hidden_dict=None)
                # encode mask results
                if isinstance(result[0], tuple):
                    result = [(bbox_results, encode_mask_results(mask_results))
                            for bbox_results, mask_results in result]
