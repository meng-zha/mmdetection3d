...
class R3D3DHead(R3DVoteHead):
    ...
    def _get_reg_out_channels(self):
        ...
        return 3 + 3 + self.num_dir_bins * 2 + 2
    ...
    def loss(self,
        ...
        targets = self.get_targets(points, gt_bboxes_3d, gt_labels_3d,
                                   gt_offset, pts_semantic_mask,
                                   pts_instance_mask, bbox_preds)
    ...
    def get_targets(self,
        ...
        for index in range(len(gt_labels_3d)):
            if len(gt_labels_3d[index]) == 0 or (gt_labels_3d[index] !=
                                                 -1).sum() == 0:
                fake_box = gt_bboxes_3d[index].tensor.new_zeros(
                    1, gt_bboxes_3d[index].tensor.shape[-1])
                gt_bboxes_3d[index] = gt_bboxes_3d[index].new_box(fake_box)
                gt_labels_3d[index] = gt_labels_3d[index].new_zeros(1)
                gt_offset[index] = gt_offset[index].new_zeros(1, 2)
    ...
    def get_targets_single(self,
        ...
        centerness_l = torch.min(distance_front, distance_back) / (
            torch.max(distance_front, distance_back) + 1e-9)
        centerness_w = torch.min(distance_left, distance_right) / (
            torch.max(distance_left, distance_right) + 1e-9)
        centerness_h = torch.min(distance_bottom, distance_top) / (
            torch.max(distance_bottom, distance_top) + 1e-9)
    ...
    def get_bboxes(self, points, bbox_preds, input_metas, rescale=False):
        ...
        for b in range(batch_size):
            bbox_selected, score_selected, labels, offset_selected = self.multiclass_nms_single(
                obj_scores[b], sem_scores[b], bbox3d[b], points[b, ..., :3], bbox_preds['offset'][b],
                input_metas[b])
            bbox = input_metas[b]['box_type_3d'](
                bbox_selected.clone(),
                box_dim=bbox_selected.shape[-1],
                with_yaw=self.bbox_coder.with_rot)
            results.append((bbox, score_selected, labels, offset_selected))

        return results

    def multiclass_nms_single(self, obj_scores, sem_scores, bbox, points, offset,
                              input_meta):
        ...
        if self.test_cfg.per_class_proposal:
            bbox_selected, score_selected, labels, offset_selected = [], [], [], []
            for k in range(sem_scores.shape[-1]):
                bbox_selected.append(bbox[selected].tensor)
                score_selected.append(obj_scores[selected])
                labels.append(
                    torch.zeros_like(bbox_classes[selected]).fill_(k))
                offset_selected.append(offset[selected])
            bbox_selected = torch.cat(bbox_selected, 0)
            score_selected = torch.cat(score_selected, 0)
            labels = torch.cat(labels, 0)
            offset_selected = torch.cat(offset_selected, 0)
        else:
            bbox_selected = bbox[selected].tensor
            score_selected = obj_scores[selected]
            labels = bbox_classes[selected]
            offset_selected = offset[selected]

        return bbox_selected, score_selected, labels, offset_selected
    ...
    def assign_seeds(self, bboxes_3d, seed_points, offset):
        """
        Args:
            bboxes_3d (BaseInstance3DBoxes): Instance of bounding boxes.
            seeds (torch.Tensor): seeds
            hidden_offset (torch.Tensor): offset of the seed points

        Returns:
            hidden_offset (torch.Tensor): offset filtered by inside boxes
            vote_assignment (torch.Tensor): track ids of the hidden seeds
        """
        batch_size = len(bboxes_3d)
        hidden_offset, assignment = [], []
        for b in range(batch_size):
            enlarged_gt_bboxes_3d = bboxes_3d[b]['boxes_3d'].enlarged_box(
                self.test_cfg.expand_dims_length)
            enlarged_gt_bboxes_3d.tensor[:, 2] -= self.test_cfg.expand_dims_length
            vote_mask, vote_assignment = self._assign_targets_by_points_inside(
